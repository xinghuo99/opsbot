...
class ScreenshotWindow(QWidget):
    ...
    def _start_audio_capture(self):
        ...
        try:
            ...
            logging.warning("pyaudio 未安装，无法采集音频")
            self.py_audio = None
            return
        except Exception as e:
            logging.warning(f"pyaudio 初始化失败: {e}")
            self.py_audio = None
            return
        ...
        try:
            ...
            if wasapi_host is not None:
                for i in range(self.py_audio.get_device_count()):
                    ...
                    if dev['hostApi'] != wasapi_host:
                        continue
                    name = dev['name'].lower()
                    if dev['maxInputChannels'] > 0 and 'loopback' in name:
                        speaker_idx = i
                        logging.info(f"找到 WASAPI 扬声器回环设备 [{i}]: {dev['name']}")
                        break

            if speaker_idx is None:
                # 回退：搜索 "Stereo Mix" 或 "What U Hear" 等虚拟设备
                for i in range(self.py_audio.get_device_count()):
                    dev = self.py_audio.get_device_info_by_index(i)
                    name = dev['name'].lower()
                    if dev['maxInputChannels'] > 0:
                        if any(kw in name for kw in ['stereo mix', 'what u hear', 'wave out', 'loopback']):
                            speaker_idx = i
                            logging.info(f"找到扬声器回退设备 [{i}]: {dev['name']}")
                            break

            # 话筒：使用默认输入设备（最可靠，兼容 MME/DirectSound/WASAPI）
            try:
                default_in = self.py_audio.get_default_input_device_info()
                if default_in['maxInputChannels'] > 0:
                    mic_idx = default_in['index']
                    logging.info(f"默认输入设备 [{mic_idx}]: {default_in['name']}")
            except Exception as e:
                logging.warning(f"获取默认输入设备失败: {e}")
            ...
            if mic_idx is None:
                for i in range(self.py_audio.get_device_count()):
                    ...
                    if dev['maxInputChannels'] > 0:
                        mic_idx = i
                        logging.info(f"使用第一个可用输入设备 [{i}]: {dev['name']}")
                        break
        except Exception as e:
            logging.warning(f"查找音频设备失败: {e}")
    ...
    def _audio_capture_loop(self):
        ...
        try:
            ...
            # 打开话筒流（默认设备，不指定 index 用系统默认输入）
            if self.record_mic_enabled:
                try:
                    kw = dict(format=fmt, channels=channels, rate=rate,
                              input=True, frames_per_buffer=chunk)
                    if mic_idx is not None:
                        kw['input_device_index'] = mic_idx
                    mic_stream = self.py_audio.open(**kw)
                    logging.info("话筒流已打开")
                except Exception as e:
                    logging.warning(f"打开话筒流失败: {e}")
                    # 回退：不指定设备索引
                    try:
                        mic_stream = self.py_audio.open(
                            format=fmt, channels=channels, rate=rate,
                            input=True, frames_per_buffer=chunk)
                        logging.info("话筒流已打开（回退默认设备）")
                    except Exception as e2:
                        logging.warning(f"打开话筒流回退也失败: {e2}")
            ...
            if self.record_speaker_enabled and speaker_idx is not None:
                try:
                    ...
                    logging.info("扬声器回环流已打开")
                except Exception as e:
                    logging.warning(f"打开扬声器回环流失败: {e}")
